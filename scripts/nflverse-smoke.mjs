...
const expected=new Set(['QB','RB','WR','TE','K']);
const unexpected=Object.keys(positions).filter(p=>!expected.has(p));
if(unexpected.length)throw new Error(`unexpected player positions after fantasy filter: ${unexpected.join(', ')}`);
for(const p of expected)if(!positions[p])throw new Error(`missing expected fantasy position: ${p}`);
