...
export function yahooPlayer(player){const p=merge(player),name=p.name||{},selected=p.selected_position||{};return{id:String(value(p.player_key)||value(p.player_id)),name:String(value(name.full)||''),position:String(value(p.display_position)||value(p.primary_position)||'').split(',')[0].toUpperCase(),nflTeam:String(value(p.editorial_team_abbr)||'').toUpperCase(),projection:0,status:String(value(p.status)||'').toUpperCase(),lineupSlot:String(value(selected.position)||value(selected)||'').toUpperCase()}}
export function yahooStanding(team){const s=team.team_standings||{},o=s.outcome_totals||{},pointsAgainst=optionalNumber(s.points_against??team.points_against),rank=optionalNumber(s.rank??team.rank),waiverPriority=optionalNumber(team.waiver_priority),moves=optionalNumber(team.number_of_moves);return{wins:Number(value(o.wins)||0),losses:Number(value(o.losses)||0),ties:Number(value(o.ties)||0),points:Number(value(team.team_points?.total)||0),...(pointsAgainst!==undefined?{pointsAgainst}:{}),...(rank!==undefined?{rank}:{}),...(waiverPriority!==undefined?{waiverPriority}:{}),...(moves!==undefined?{moves}:{})}}
