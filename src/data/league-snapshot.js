...
export function validateLeagueSnapshot(input){
 let league;try{league=normalizeLeagueSnapshot(input)}catch(error){return{valid:false,errors:[error.message],warnings:[],league:null}}const errors=[],warnings=[],ids=new Set();if(league.teams.length<2)errors.push('At least two teams are required.');for(const team of league.teams){if(ids.has(team.id))errors.push(`Duplicate team id: ${team.id}`);ids.add(team.id);if(team.wins<0||team.losses<0||team.ties<0)errors.push(`Team ${team.id} has a negative record value.`);const all=[...team.lineup,...(team.roster||[]),...Object.values(team.weeklyLineups||{}).flat()];if(!team.lineup.length&&!team.roster?.length&&!Object.keys(team.weeklyLineups||{}).length)warnings.push(`Team ${team.id} has no lineup data.`);for(const p of all){if(!POSITIONS.has(p.position))errors.push(`Player ${p.id} on ${team.id} has unsupported position '${p.position}'.`);if(p.projection<0)errors.push(`Player ${p.id} on ${team.id} has a negative projection.`)}}const seenWeeks=new Set();for(const week of league.schedule){if(!Number.isInteger(week.week)||week.week<1)errors.push(`Invalid schedule week: ${week.week}`);if(seenWeeks.has(week.week))errors.push(`Duplicate schedule week: ${week.week}`);seenWeeks.add(week.week);const playing=new Set();for(const[a,b]of week.matchups){if(!ids.has(a)||!ids.has(b))errors.push(`Week ${week.week} matchup references unknown team: ${a} vs ${b}.`);if(a===b)errors.push(`Week ${week.week} has self-matchup for ${a}.`);if(playing.has(a)||playing.has(b))errors.push(`Week ${week.week} schedules a team more than once.`);playing.add(a);playing.add(b)}}if(!league.schedule.length)errors.push('At least one remaining regular-season schedule week is required.');if(league.playoffSpots<2||league.playoffSpots>league.teams.length)errors.push('playoffSpots must be between 2 and the number of teams.');if(!league.playoffWeeks.length)errors.push('At least one playoff week is required.');return{valid:errors.length===0,errors,warnings,league};
}
export function importLeagueSnapshot(input,{strict=true}={}){const result=validateLeagueSnapshot(input);if(strict&&!result.valid)throw new Error(`Invalid league snapshot:\n- ${result.errors.join('\n- ')}`);return result}
