...
export function joinHistoricalProjectionActuals(projections,actuals,{allowDST=false}={}){const p=allowDST?projections:projections.filter(r=>r.position!=='DST'&&r.position!=='DEF'),a=allowDST?actuals:actuals.filter(r=>r.position!=='DST'&&r.position!=='DEF');const joined=joinProjectionActuals(p,a);return{...joined,summary:{projectionRows:p.length,actualRows:a.length,matched:joined.rows.length,unmatchedProjections:joined.reconciliation.unmatchedProjections.length,unmatchedActuals:joined.reconciliation.unmatchedActuals.length}};}
