...
function summary(r){return `${r.label}: ${pct(r.playoffDelta)} playoff, ${pct(r.championshipDelta)} championship, ${wins(r.winsDelta)} wins`;}

export function buildWeeklyGMRecommendations(input,{teamId,week,projectionRows=[],waivers=null,trades=[],startSit=true,limit=10},options={}){
 ...
 const ranked=actions.sort(compare).map((r,i)=>({...r,rank:i+1,summary:summary(r)}));
 return {teamId:team.id,teamName:team.name,week:Number(week),simulations:input.simulations,seed:input.seed,actionCount:ranked.length,recommendations:ranked.slice(0,limit)};
}
