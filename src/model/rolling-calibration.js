...
export function runRollingCalibration(rows,{draws=1500,seed=20260923,minTrainingRows=100,minPositionRows=30}={}){
 ...
 if(!folds.length||!allFixed||!allEmp||!allFixed.n||!allEmp.n)return{rows:ordered.length,folds,comparison:null,byPosition:{fixed:fixedSummary,empirical:empiricalSummary},recommendation:'insufficient-data',adoptEmpirical:false,latestParameters:null};
 const comparison={fixed:{...allFixed,calibrationPenalty:calibrationPenalty(allFixed)},empirical:{...allEmp,calibrationPenalty:calibrationPenalty(allEmp)}};
 const adopt=allEmp.rmse<=allFixed.rmse&&comparison.empirical.calibrationPenalty<=comparison.fixed.calibrationPenalty;
