...
export function prepareLeagueSimulation(snapshot,projectionRows,{slots=DEFAULT_SLOTS,weeks=null,season=snapshot?.source?.season,strictProjections=false,minimumProjectionMatchRate=.9,calibrationReport=null,simulations=50000,seed=20260923,modelVariant='correlated'}={}){
 const enriched=enrichWeeklyProjections(snapshot,projectionRows,{weeks,season,strict:strictProjections});
 const optimized=buildFutureWeeklyLineups(enriched.league,{slots,weeks,useRoster:true});
 const incomplete=[];
 ...
 input.metadata={...input.metadata,rosterProjectionCoverage:enriched.coverage,incompleteLineups:incomplete};
 return {league:optimized,input,diagnostics:{projectionCoverage:enriched.coverage,incompleteLineups:incomplete}};
}
